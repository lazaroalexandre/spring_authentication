...
public class UserService {
    ...
    public UserSaveDto save(UserSaveDto userDto) {
        ...
        UserSaveDto newUserDto = new UserSaveDto(userDto.id(), userDto.name(), userDto.email(), encryptedPassword,
    ...
    public UserTokenDto login(UserSaveDto userDto) {
        ...
        boolean passwordValid = new BCryptPasswordEncoder().matches(userDto.password(), user.get().password());

        if (user == null || user.get().valid() == false || !passwordValid) {
        ...
        return userRepository.findById(id).map(recordFound -> {
            if (recordFound.getRole() == Role.USER) {
                ...
                if (saveUserDto.password() != null) {
                    String encryptedPassword = new BCryptPasswordEncoder().encode(saveUserDto.password());
                    recordFound.setPassword(encryptedPassword);
                }
